...
is not the current byte inventory. A refreshed manifest and frozen release remain
...
inputs are ineligible evidence.  In particular, the Round-09 theorem is a
...
novelty evidence.
Campbell's 1999 dissertation already records the standard two-isogeny method;
the claimed contribution is only the exact target-specific computation. The
Round-12 audit and explicitly unexecuted Sage/PARI/Magma protocol are stored
in `../NEXT_ELLIPTIC_ROUND_12_NOVELTY_AND_SECOND_CAS_AUDIT.md` and
`../notes/independent-second-cas-plan.md`.
...
The historical v0.6.1 supplement manifest predates the Round-10 files; a refreshed
