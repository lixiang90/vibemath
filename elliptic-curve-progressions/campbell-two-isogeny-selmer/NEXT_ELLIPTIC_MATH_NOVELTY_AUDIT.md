...
2. Campbell's 1999 Rutgers dissertation explicitly gives the standard
   rational two-isogeny descent for `y^2=x^3+a*x^2+b*x`, its dual curve,
   quartic coverings, two Selmer groups, and rank formula. The method is
   prior art and is not claimed as a contribution:
...
3. Bremner's 1999 article is the earlier length-8 Weierstrass-family source:
   https://doi.org/10.1080/10586458.1999.10504629
4. Garcia-Selfa--Tornero study simultaneous arithmetic progressions, not
   this numerical descent:
   https://doi.org/10.1017/S0004972700038429
5. Garcia-Fritz--Pasten prove a general rank-growth theorem for long
   progressions in fixed-j families, not the rank or Selmer groups of this
   curve:
   https://doi.org/10.1093/imrn/rnaa061
6. Fisher gives the binary-quartic Selmer description and the exact cubic
   invariant used here:
   https://doi.org/10.1007/s40993-022-00376-z
7. Fisher's rational-2-torsion paper is the relevant higher-descent context:
...
Do not use “first”, “new descent”, “new solution of the nine-term problem”,
“rank of the Campbell curve”, or “Cassels--Tate obstruction”. The descent
formalism already appears in Campbell's dissertation. The minimal-model and
isomorphism-class part of this earlier risk assessment is discharged in
Section 6 below. The remaining priority risks are the unavailable complete
MathSciNet citation graph, incomplete old zbMATH metadata, and non-indexed
literature or code.
...
passes **73/73 tests**.  No Magma, Sage, PARI/GP, remote CAS, or claimed
...
Priority assessment after Round 12: **medium residual risk for priority but
moderate confidence in the target-specific not-found result**. The lack of an
LMFDB row is expected from its published coverage and is not novelty
evidence. Campbell's dissertation has now been inspected and confirms the
method as prior art. The complete MathSciNet citation graph was unavailable,
old zbMATH metadata are incomplete, and neither the original model nor its
global minimal isomorphism class was located in the audited indexed material.
...
## 7. Round-09 through Round-11 artifacts and claim boundary
...
of the 73-test command above.  The complementary Round-10 `E'`-side theorem is
...
Round 11 is generated by code/NEXT_ELLIPTIC_ROUND_11.py, frozen in
certificates/round11_isogeny_selmer_audit.json, and checked by
code/NEXT_ELLIPTIC_ROUND_11_test.py. It supplies the convention/support
bridge that upgrades the two proved local survivor lists to the exact
isogeny Selmer groups, revalidates all 84 finite positive primitive witnesses
for the twelve surviving classes, and derives only the upper bound
rank E(Q) <= 3.
...
## 8. Round-12 novelty and independent-CAS audit

`NEXT_ELLIPTIC_ROUND_12_NOVELTY_AND_SECOND_CAS_AUDIT.md` records MR1971433,
zbMATH document 1919531 / Zbl 1022.11026, Campbell's article and dissertation,
exact-isomorphism keys, and database coverage boundaries. Its conclusion is
asymmetric: the standard method is old, while no indexed source was located
for this exact target-specific pair of Selmer groups. That is a
moderate-confidence not-found assessment, not a priority theorem.

The executable future protocol `notes/independent-second-cas-plan.md`
enumerates signed squarefree divisors on both models with Sage's exact
`test_els`, then checks orders 8 and 4 with
`two_descent_by_two_isogeny(..., proof=True)`. PARI/GP gives only weaker
full-2-Selmer/rank-interval corroboration; locally licensed Magma is an
optional third check. None was available or executed, so no independent-CAS
