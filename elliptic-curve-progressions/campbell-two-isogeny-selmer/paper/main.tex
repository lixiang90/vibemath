...
Mordell--Weil rank at most $3$.  We also compute the full $\Q\times K$
...
regression tests.  The same-parameter genus-$5$ fibre product is everywhere
...
certificate contains all $64\times8=512$ local cells.  A YES cell contains
...
b7df31fc2bd81f99e2e4d00095233be45e9659438014f12f8326a597ad3e40ed.
\end{quote}
The structured $\Q\times K$ and Selmer certificate has SHA-256
\begin{quote}\small\ttfamily
ae6e8a3895cc8a09bb6e3f7fc23904e4adfabd7fc9738b8c428150db1650c431.
\end{quote}
...
SymPy versions, and exact reproduction commands.  Its archive URL is null;
this is a local release candidate rather than a public archive.
...
places are automatic by Lemma~\ref{lem:good}.  The certificate checks the
remaining finite and real places and yields exactly the survivor sets in
Table~\ref{tab:local-classification}.  Direct squareclass multiplication
...
rational $2$-torsion projection $[35]$.  Moreover the same-parameter
...
the full $2$-Selmer group or any Cassels--Tate value.
...
\path{paper-elliptic-campbell-supplement-v0.6.1}.  They are currently sealed
only in the local deterministic archive candidate; before submission the
authors must deposit those exact bytes and provide a verified persistent DOI
or URL: \texttt{<<PERSISTENT DOI/URL REQUIRED>>}.
