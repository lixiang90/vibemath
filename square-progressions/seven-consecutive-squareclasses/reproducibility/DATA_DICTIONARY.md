...
Mathematical release described: `paper-square-supplement-v0.9.0`.  The earlier
...
Supersession: the mask-99 certificate binds this 23-row output and supplies the
next complete exclusion.

## `PAPER_SQUARE_MASK99_CERTIFICATE.json`

Schema/version: `paper-square-mask99-v1`, semantic version `1.0.0`.

- `input_sha256` binds the mask-108 23-row output and the Round-04 occurrence
  table.
- `occurrence_inventory` records all 345 character occurrences, all 55
  distinct masks, each mask's support/degree/genus/hit IDs, and the exact
  ranking of genus-one masks with a constant quadratic pairing.
- `integral_points` stores the mask-99 equation, the `A,B` pairing, gcd bound,
  squarefree kernels `1,5`, finite middle interval and complete six-point list.
- `pattern_impact` stores units of 23 input patterns, eight excluded IDs and 15
  surviving necessary IDs.

Claim eligibility: exact evidence for the mask-99 integral-point theorem and
the 23-to-15 exclusion. `bounded_search_used=false`; no search range supports
...
Supersession: the mask-90 certificate binds this ten-row output and supplies
the next complete exclusion.

## `PAPER_SQUARE_MASK90_CERTIFICATE.json`

Schema/version: `paper-square-mask90-v1`, semantic version `1.0.0`.

- `input_sha256` binds the mask-51 ten-row output, the Round-04 occurrence
  table and the Round-02 canonical partition table.
- `occurrence_inventory` independently reconstructs 150 character
  occurrences, 49 masks, 25 genus-one masks, the pairable-mask ranking and the
  three exact mask-90 occurrence records.
- `integral_points` stores the mask-90 equation, the constant-six pairing,
  all four squarefree-kernel branches, the exhaustive sign-complement interval
  and the complete four branch points.
- `pattern_impact` stores units of ten input patterns, three excluded IDs and
  seven surviving necessary IDs with their canonical partitions.
- `bounded_search_used=false` and `mordell_weil_used=false` are explicit
  claim-boundary flags.

Claim eligibility: exact evidence for the complete mask-90 integral-point
theorem and the 10-to-7 exclusion.  It does not assert realizability of a
remaining pattern or decide `R_2(7)`.

Supersession: the mask-54 certificate binds this seven-row output and supplies
the next complete exclusion.

## `PAPER_SQUARE_MASK54_CERTIFICATE.json`

Schema/version: `paper-square-mask54-v1`, semantic version `1.0.0`.

- `input_sha256` binds the mask-90 seven-row output, the already proved
  mask-108 certificate, the Round-04 occurrence table and the Round-02
  canonical partition table.
- `occurrence_inventory` independently reconstructs 105 character
  occurrences, 41 masks, 22 genus-one masks, the complete remaining
  constant-pairable ranking and the three exact mask-54 occurrence records.
- `integral_points.translation_to_mask108` stores the factorwise identity,
  mutually inverse integer translations and point-by-point bijection with the
  certified mask-108 list.
- The rest of `integral_points` is an independent constant-three pairing
  proof: both squarefree kernels, the exhaustive five-integer sign-complement
  interval, the six-point list and the zero position in the original block.
- `pattern_impact` stores seven input patterns, IDs `59,214,230` excluded at
  the same normalized parameter, and the four surviving canonical partitions.
- `bounded_search_used=false` and `mordell_weil_used=false` are explicit
  claim-boundary flags.

Claim eligibility: exact evidence for the complete mask-54 integral-point
