...
|---|---|---|
| `square-progressions/seven-consecutive-squareclasses` | internally accepted + Round09 gate | For integer `t`, the equality patterns of the nonzero block `t,...,t+6` with affine rational squareclass rank at most two are reduced through the exact chain `651 -> 343 -> 284 -> 98 -> 54 -> 35 -> 23 -> 15 -> 10 -> 7`.  Realizability or impossibility of the final 7 patterns, and therefore `R_2(7)`, remain open. |
| `powers-in-progressions/pure-cubic-five-term` | internally accepted + Round09 extension | `R^times_(3,1)(5)=4`; four of the 31 four-hit models now yield infinite families, and all remaining 29 inputs were reconstructed into 25 clusters under the explicitly stated permutation action.  The other 27 models remain open. |
| `elliptic-curve-progressions/campbell-two-isogeny-selmer` | internally accepted + Round09 local gate | Exact two-isogeny Selmer groups and `rank <= 3`; an explicit global minimal model has conductor `301245307115205810`.  The real place together with `p=59` or `71699` reduces the 32 `E`-side support classes to 8 necessary classes.  No ninth rational point, global obstruction, full 2-Selmer group, Cassels--Tate value, or rank equality is claimed. |
