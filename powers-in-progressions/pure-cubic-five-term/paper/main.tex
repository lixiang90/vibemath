...
prove that the four-hit orbit with positions $0,1,3,4$ and colors $0001$ is a
positive-rank genus-one branch.  It supplies infinitely many inequivalent
four-hit progressions, beginning with $(64,1,-62,-125,-188)$ over
$\Q(\sqrt[3]{188})$.
\end{abstract}
...
\emph{five-hit color/position class}.  The next section settles one of the 31
four-hit models left by the orbit sieve; it does not classify every rational
progression attaining four hits.  The other 30 models remain distinct
...
\section{A positive-rank four-hit branch}

We now strengthen Theorem~\ref{thm:main} in one, and only one, of the 31
four-hit color/position models left open above.  Count positions $0,1,3,4$
and take colors $0001$.  Writing the first three counted terms as
$X^3,Y^3,Z^3$, respectively, gives the smooth plane cubic
\begin{equation}\label{eq:C0001}
C:\qquad 2X^3-3Y^3+Z^3=0.
\end{equation}
Indeed the associated five-term progression is
...
The branch \eqref{eq:C0001} has positive Mordell--Weil rank.  Consequently
...
positions in a nontrivial pure cubic field.  They may all be taken in the
color/position orbit
\[
((0,1,3,4),0001).
\]
\end{theorem}
...
cube-free representative $D_n$ of $[A_4]$.  Then
$A_4=D_nW_n^3$ for some $W_n\in\Q^*$, and $K_{D_n}$ is genuinely cubic.
Thus the four displayed positions have colors $0001$.  Theorem
\ref{thm:main} prevents the remaining position from being a fifth hit.
...
\[
 (64,1,-62,-125,-188),
\]
with common difference $-63$.  If $\alpha^3=188$, its four counted terms are
$4^3,1^3,(-5)^3,(-\alpha)^3$.  This differs from the lower-bound progression
...
omits position 2 and lies in the orbit $0001$ over
$\Q(\sqrt[3]{188})$.
...
exclusion of all five-hit color classes, and a positive-rank infinite family
in the single four-hit orbit $0001$.  We make no priority claim and no
classification claim for the other 30 four-hit models.
