# Journal shortlist

Checked 2026-09-03. This is a scope screen, not a prediction of acceptance;
all policies must be rechecked immediately before submission.

## 1. Research in Number Theory — preferred current fit

Official page: <https://link.springer.com/journal/40993>

The journal describes its scope as number theory and arithmetic geometry and
publishes original articles in traditional areas. The Kummer kernel,
Diophantine progression theorem and explicit local obstructions fit directly.
The main risk is that the present manuscript is only four pages; a human
author should decide whether to expand motivation, related work and certificate
exposition before submission. Verify the then-current AI and supplement policy.

## 2. Journal of Number Theory — ambitious backup

Official page: <https://www.sciencedirect.com/journal/journal-of-number-theory>

Its broad number-theory remit is relevant, including computational number
theory. The likely editorial risk is depth/length rather than subject mismatch.
This option becomes more plausible after an independent mathematical referee
confirms the kernel and prime-support arguments and the exposition is expanded.

## 3. Electronic Journal of Combinatorics — conditional framing

Official policy: <https://www.combinatorics.org/ojs/index.php/eljc/about>

The exact 25-orbit classification and 60-cell finite certificate provide a
genuine combinatorial component. The journal states high standards and expects
substantial discrete-mathematics content, so this is conditional on foregrounding
the orbit action and finite classification. Its 2026 AI policy allows assistance
but puts full checking and attribution responsibility on human authors, which is
compatible only after the checklist is completed.

## Explicitly excluded under the current policy: INTEGERS

The 2026 author instructions at
<https://math.colgate.edu/~integers/submit.html> say that the journal will not
consider an article using AI in producing mathematics, code, bibliography or
other content. That conflicts with this project's accurate disclosure. Do not
submit there unless the policy changes materially; do not misdescribe the role
of AI to obtain eligibility.

Target journal remains `<<TARGET JOURNAL — USER DECISION>>`.
